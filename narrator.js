/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Islam wal-Awda Al-Iqtisadiyah
...
(function() {
  ...
  const STATE = { playing:false, paused:false, mode:'page', cardIndex:0, cards:[], tabOrder:['about','cards','econ'], tabIndex:0, loopCount:0, loopCurrent:0, sleepTimer:null, sleepMinutes:0, duoTimeout:null, karaokeEnabled:true, autoScroll:true, duoReading:false, speed:1, pitch:1, voiceAR:null, voiceEN:null, voiceFR:null };
  ...
  function extractCards(panel) {
    ...
    } else if (tabName === 'econ') {
      panel.querySelectorAll('.anxiety-card').forEach(el => { cards.push({ el, text: cleanText(el.textContent), type: 'econ' }); });
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    if (!duoText && card.type === 'adhkar' && typeof ECON_DATA !== 'undefined') { const ac = Array.from(card.el.parentNode.querySelectorAll('.anxiety-card')); const idx = ac.indexOf(card.el); if (idx >= 0 && ECON_DATA[idx] && ECON_DATA[idx].fr) { const a = ECON_DATA[idx].fr; duoText = (a.title||'') + '. ' + (a.problem||'') + '. ' + (a.solution||''); } }
  ...
  function onSpeedChange(v){STATE.speed=parseFloat(v);const l=document.getElementById('narratorSpeedLabel');if(l)l.textContent=STATE.speed+'x';localStorage.setItem('ie-narrator-speed',STATE.speed);}
  function onPitchChange(v){STATE.pitch=parseFloat(v);const l=document.getElementById('narratorPitchLabel');if(l)l.textContent=STATE.pitch.toFixed(1);localStorage.setItem('ie-narrator-pitch',STATE.pitch);}
  ...
  function onKaraokeToggle(c){STATE.karaokeEnabled=c;localStorage.setItem('ie-narrator-karaoke',c);}
  function onAutoScrollToggle(c){STATE.autoScroll=c;localStorage.setItem('ie-narrator-autoscroll',c);}
  function onDuoToggle(c){STATE.duoReading=c;localStorage.setItem('ie-narrator-duo',c);}
  ...
  function loadSettings(){STATE.speed=parseFloat(localStorage.getItem('ie-narrator-speed'))||1;STATE.pitch=parseFloat(localStorage.getItem('ie-narrator-pitch'))||1;STATE.karaokeEnabled=localStorage.getItem('ie-narrator-karaoke')!=='false';STATE.autoScroll=localStorage.getItem('ie-narrator-autoscroll')!=='false';STATE.duoReading=localStorage.getItem('ie-narrator-duo')==='true';}
  ...
  var origRenderAdhkar=window.renderEcon;if(origRenderAdhkar){window.renderEcon=function(){origRenderAdhkar();setTimeout(injectSpeakButtons,100);};}
